...
const FlowArt: React.FC<{ children: React.ReactNode; className?: string; 'aria-label'?: string }> = ({ children, className, 'aria-label': ariaLabel = 'Story scroll' }) => {
  ...
  useGSAP(() => {
    ...
    sections.forEach((section, i) => {
      ...
      if (i < sections.length - 1) {
        triggers.push(ScrollTrigger.create({ trigger: section, start: 'bottom bottom', end: 'bottom top', pin: true, pinSpacing: false }));
      }
    });
